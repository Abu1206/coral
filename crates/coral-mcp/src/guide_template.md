...
- `coral.columns` may include `null_fraction`, `approx_distinct_count`, `stats_sample_count`, `stats_observed_at`, and `stats_precision`. These fields are nullable, may be based on observed samples, and should not be treated as exact unless `stats_precision = 'exact'`.
