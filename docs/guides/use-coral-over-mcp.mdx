...
`coral.columns` also exposes nullable column-statistics fields such as `null_fraction`, `approx_distinct_count`, `stats_sample_count`, `stats_observed_at`, and `stats_precision` when Coral has observed a table through local query execution. These values may be sample-derived, and unsupported or awkward column types may stay `NULL`; agents should treat them as hints unless `stats_precision` says `exact`.
