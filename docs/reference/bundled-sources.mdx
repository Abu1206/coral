...
Connect with Notion OAuth or provide an internal integration token.
For OAuth, create a public Notion connection with this redirect URI:
http://127.0.0.1:53682/oauth/callback. Then provide the OAuth client ID
and client secret when prompted.

For a token, create an internal Notion integration and copy its internal
integration token. Share the pages and databases you want to query with
the integration from Notion's "Add connections" menu.
