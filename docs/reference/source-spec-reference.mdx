...
used. OAuth supports device-code flow and authorization-code flow.

For device-code flow, declare `flow.type: device_code`,
`endpoints.device_authorization_url`, and `endpoints.token_url`. Device-code
OAuth uses a public client ID only: `client.secret` must not be declared, and
no callback URL, loopback listener, client secret, or PKCE setting is required.

For authorization-code flow, declare `flow.type: authorization_code`, `pkce`,
`redirect_uri`, `endpoints.authorization_url`, and `endpoints.token_url`.
Authorization-code flow uses a loopback HTTP callback URI:
